...
export default async function handler(
  ...
  try {
    ...
    // FIXED: Changed 'status' to 'botStatus' to avoid duplicate
    res.status(200).json({ 
      botStatus: "Running",  // ← THIS LINE WAS FIXED
      session: {
...
async function runTradingTick(session: any) {
  try {
    ...
    // FIXED: Changed 'status' to 'tradeStatus' to avoid duplicate
    return {
      tradeStatus: "success",  // ← THIS LINE WAS FIXED
      input: "0.01 SOL",
      output: `${uiPrice.toFixed(4)} USDC`,
      route: data?.routes?.[0],
    };
  } catch (err: any) {
    return { tradeStatus: "error", message: err.message };  // ← THIS LINE WAS FIXED
  }
}
